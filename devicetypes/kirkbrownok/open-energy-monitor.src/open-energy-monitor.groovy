...
def parse(String message) {
    ...
    if (msg.headers) {
        ...
        def csv = "${state.meter},${tstat.EMpower[0].toFloat()},${csvTime-numOfSamples}"
        ...
        for (i in 1..numOfSamples - 1) {          	
                ...
                if (state.removeCounter == removeEvery){
                	TRACE("removed:${state.meter},${tstat.EMpower[i].toFloat()},${csvTime-10+i}")
                    state.removeCounter = 1
                } else {
                	csv = "${csv},${state.meter},${tstat.EMpower[i].toFloat()},${csvTime-10+i}"
                    state.removeCounter = state.removeCounter +1
                }
		}
        //POST /api/v2/push_readings HTTP/1.1\r\nAuthorization: Basic WVRVNFpqa3lPRE0yTnprNDo\r\nHost: plotwatt.com\r\nContent-Length: XXXX\r\n\r\n
        //TRACE("csv: \r\n${csv}")
        def postHeaders = [ 
        Host : "tnrtkrucm4ig.runscope.net",
        Authorization : "Basic TkRVNFpEQTFabVV6WXpVNTo="
        ]
                //"Host": "plotwatt.com",	
        def postParams = [
